        ...
        {[...Array(30)].map((_, i) => (
          <motion.div
            ...
            animate={{
              ...
              opacity: [0, 0.2, 0],
              scale: [Math.random() * 1.5, Math.random() * 2, Math.random() * 1.5],
            }}
            transition={{
      ...
      <div className='relative z-10 min-h-screen flex items-center justify-center p-6'>
        ...
        >
          <div className='text-center mb-6'>
            ...
            <h2 className='text-4xl font-bold text-white drop-shadow-lg'>Prize Configuration</h2>
            <p className='text-white/90 mt-2'>Set up your competition prizes</p>
          </div>

          <div className='space-y-5'>
            {['participants', 'firstPrize', 'secondPrize', 'thirdPrize'].map((field, index) => {
              const icons = [
                <UserIcon key={uid()} className='w-5 h-5 mr-2 text-[#2575fc]' />,
                <TrophyIcon key={uid()} className='w-5 h-5 mr-2 text-yellow-400' />,
                <GiftIcon key={uid()} className='w-5 h-5 mr-2 text-green-400' />,
                <GiftIcon key={uid()} className='w-5 h-5 mr-2 text-bronze-400' />,
              ];
              const labels = [
